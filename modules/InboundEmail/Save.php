...
if ( $focus->isMailBoxTypeCreateCase() || ($focus->mailbox_type == 'createcase' && empty($_REQUEST['id']) ) )
{
	...
	/*Alex assign case*/
	$assigned_uids=(isset($_REQUEST['create_case_user_id'])) ? implode(',',$_REQUEST['create_case_user_id']) : "";
	$stored_options['create_case_user_id'] = $assigned_uids;
	$assigned_uids_emails_skip=(isset($_REQUEST['create_case_skip_emails'])) ? implode(',',$_REQUEST['create_case_skip_emails']) : "";
	$stored_options['create_case_skip_emails'] = $assigned_uids_emails_skip; 
	/*Alex assign case*/
} // if
