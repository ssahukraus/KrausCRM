...
function generateComposeDataPackage($data,$forFullCompose = TRUE, $bean = null)
{
	...
	!isset($data['ListView']) && !isset($data['replyForward'])) {
		...
		if ($bean->module_dir == 'Cases') {
			...
			/*
			 *@author kbedi@kraususa.com
			 *@todo Update later to check all case relationships to retreive proper email address
			 * Check if cases is linked to a contact, if not assume that its linked to an account and retieve information from account
			*/
			if(count($contact_ids) > 0){
				...
				$account_id = $bean->accounts->get();
				$account = new Account();
                        	$account->retrieve($account_id[0]);
				$namePlusEmail .= empty($namePlusEmail) ? "" : ", ";
                                $namePlusEmail .= from_html($account->name) . " <".from_html($account->email1).">";
			}
