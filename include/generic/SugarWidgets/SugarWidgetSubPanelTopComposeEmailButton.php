...
class SugarWidgetSubPanelTopComposeEmailButton extends SugarWidgetSubPanelTopButton
{
	...
	function display($defines)
	{
		...
		if($userPref != '') {
		 ...
		 $moduleName = $defines['focus']->object_name;
		if($client != 'sugar') {
			$bean = $defines['focus'];
			...
			/*suraj
			*to populate the mail Id in "TO" field of compose for the customers
			*/
	     if ($moduleName == "kr_Dealers") {
                $partner = BeanFactory::getBean($moduleName, $defines['focus']->id);
                ...
                $j_quickComposeOptions_decoded->composePackage->to_email_addrs = $sendtoId;
