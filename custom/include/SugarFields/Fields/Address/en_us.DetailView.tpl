...
<input type="hidden" class="sugar_field" id="country_code_c" value="{$fields.country_code_c.value|escape:'htmlentitydecode'|escape:'html'|url2html|nl2br}">
...
{$fields.country_code_c.value|escape:'htmlentitydecode'|escape:'html'|url2html|nl2br} <br/>
 
{if ( {{$displayParams.key}}=='shipping'  )  && (  !$fields.{{$displayParams.key}}_address_street.value ||  !$fields.{{$displayParams.key}}_address_city.value || !$fields.country_code_c.value || !$fields.{{$displayParams.key}}_address_postalcode.value || !$fields.{{$displayParams.key}}_address_state.value ) }
<div style="color:#FF0000;font-size:14px;">
Following address information is required:<br/>
{if !$fields.{{$displayParams.key}}_address_street.value }
 Street<br/>
{/if}
{if !$fields.{{$displayParams.key}}_address_city.value }
 City<br/>
{/if}
{if !$fields.{{$displayParams.key}}_address_state.value }
 State <br/>
{/if}
{if !$fields.country_code_c.value}
 Country <br/>
{/if}
{if !$fields.{{$displayParams.key}}_address_postalcode.value }
 Postal Code <br/>
{/if}
</div>
{/if}
