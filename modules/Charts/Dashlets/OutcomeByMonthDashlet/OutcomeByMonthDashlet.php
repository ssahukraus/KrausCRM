...
class OutcomeByMonthDashlet extends DashletGenericChart
{
    ...
    protected function constructQuery()
    {
        $query = "SELECT sales_stage,".
            db_convert('cases.date_closed','date_format',array("'%Y-%m'"),array("'YYYY-MM'"))." as m, ".
            "sum(amount_usdollar/1000) as total, count(*) as opp_count FROM cases ";
        $query .= " WHERE cases.date_closed >= ".db_convert("'".$this->obm_date_start."'",'date') .
                        " AND cases.date_closed <= ".db_convert("'".$this->obm_date_end."'",'date') .
                        " AND cases.deleted=0";
        if (count($this->obm_ids) > 0)
            $query .= " AND cases.assigned_user_id IN ('" . implode("','",$this->obm_ids) . "')";
        $query .= " GROUP BY sales_stage,".
                        db_convert('cases.date_closed','date_format',array("'%Y-%m'"),array("'YYYY-MM'")) .
                    " ORDER BY m";
